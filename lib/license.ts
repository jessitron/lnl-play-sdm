...
export const
    LicenseAspect: AspectWithReportDetails<LicenseData> = {
        name: "gh-license",
        displayName: "License",
        extract: async (p, pli) => {
        ...
        apply: async (p, papi) => {
            ...
            if (!licenses.body) {
                logger.error("No body to licenses request")
                return p;
            }
            const license = licenses.body.find(l => l.key === key);
            if (!license) {
                logger.error("No license found for key <" + key + ">")
                return p;
            }
            ...
            if (!licenseText.body) {
                logger.error("No license text body")
                return p;
            }
            const licenseFile = await p.getFile("LICENSE");
            if (!!licenseFile) {
                logger.info("Replacing license file with " + key)
                await licenseFile.setContent(licenseText.body.body);
            } else {
                logger.info("Adding license file " + key);
        ...
        details: {
            ...
            displayName: "License according to GitHub",
            unit: "gh-license",
            category: "GitHub",
